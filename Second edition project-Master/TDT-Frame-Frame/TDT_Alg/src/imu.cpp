...
void ImuCalc::TDT_IMU_update(float half_T)
{
	...
	if(norm)
	{
		...
		err.exInt = err.exInt + ex *Ki *2 *half_T;								  //对误差进行积分
		err.eyInt = err.eyInt + ey *Ki *2 *half_T;
		err.ezInt = err.ezInt + ez *Ki *2 *half_T;
		...
		quaternion.q0 = quaternion.q0 + (-quaternion.q1*gx - quaternion.q2*gy - quaternion.q3*gz) *half_T;
		quaternion.q1 = quaternion.q1 + ( quaternion.q0*gx + quaternion.q2*gz - quaternion.q3*gy) *half_T;
		quaternion.q2 = quaternion.q2 + ( quaternion.q0*gy - quaternion.q1*gz + quaternion.q3*gx) *half_T;
		quaternion.q3 = quaternion.q3 + ( quaternion.q0*gz + quaternion.q1*gy - quaternion.q2*gx) *half_T;
